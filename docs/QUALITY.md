...
| Tables | `npm run corpus:run:tables`, `npm run qa:table-detection`, and related table adjacency/span/CSV checks when touched; detection precision and recall use one-to-one matches on table format and normalized cells rather than counts alone. |
